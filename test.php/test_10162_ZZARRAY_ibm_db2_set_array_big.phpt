--TEST--
XML i Toolkit: IBM_DB2 result set SRVPGM - array big data
--SKIPIF--
...
$stmt = db2_prepare($conn, "call $procLib.iPLUGR512K(?,?,?)");
...
$myMax1  = 96;        // expected max
$myCount1= 96;        // expected count
  ...
  for ($j=0;$j<4;$j++) {
    ...
    if ((string)$actual != $expect) {
      if ($myMax > 10 && $j = 1) {
        $len1 = strlen($actual);
        $len2 = strlen($expect);
        die("Fail dcRec_t[$i].$var ($len1 != $len2) ($lib/$name.$func)\n");
      }
      else die("Fail dcRec_t[$i].$var ($actual != $expect) ($lib/$name.$func)\n");
    }
  }
  $i++;
}
...
<pgm name='ZZSRV' lib='xyzlibxmlservicexyz' func='ZZARRAY'>
 ...
 <parm comment='max allowed return'>
  <data var='myMax' type='10i0'>96</data>
 </parm>
