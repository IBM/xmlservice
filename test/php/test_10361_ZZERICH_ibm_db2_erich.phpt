...
foreach ($dsall as $ds) {
  ...
  $d       = $ds->ds;
  for ($j=0;$j<15;$j++) {
    $vkalw1  = (string)$d[$j]->data[0];
    $ds2[]   = $vkalw1;
  }
  for ($j=0;$j<15;$j++) {
    $vvsw1   = (string)$d[$j+15]->data[0];
    $ds2[]   = $vvsw1;
  }
  for ($j=0;$j<15;$j++) {
    $vvsk1   = (string)$d[$j+30]->data[0];
    $ds2[]   = $vvsk1;
  }
  // any differences?
 ...
 <parm  io='both'>
  <ds var='vevsods' dim='200'>
   ...
   <data var='vtxt' type='30A'>hallo</data>
    <ds dim='15'>
     <data var='vkalw' type='9s2'>9.2</data>
    </ds>
    <ds dim='15'>
     <data var='vvsw' type='9s2'>8.2</data>
    </ds>
    <ds dim='15'>
     <data var='vvsk' type='1s0'>1.0</data>
    </ds>
  </ds>
 </parm>
 <return>
