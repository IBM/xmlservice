--TEST--
XML i Toolkit: IBM_DB2 check cmd exe error on
--SKIPIF--
...
if (strpos($clobOut,"<error>CPF")>0) die("Error contains <error>CPFxxx</error>, but should not exec='cmd' error='fast'\n");
...
<?xml version='1.0'?>
<script>
<cmd exec='cmd' error='fast'>ADDLIBLE LIB(QTEMP)</cmd>
<cmd exec='cmd' error='fast'>ADDLIBLE LIB(QTEMP)</cmd>
</script>
ENDPROC;
return $clob;
