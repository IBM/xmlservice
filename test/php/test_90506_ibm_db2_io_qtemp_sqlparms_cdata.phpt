--TEST--
XML i Toolkit: IBM_DB2 inout SQL - qtemp prepare parm
--SKIPIF--
...
$parms = $xmlobj->xpath('/script/sql/describe/parm');
if (!$parms) die("Missing XML parms info");
$format = "Parameter: %'.15.15s \n";
foreach($parms as $parm) {
  printf ($format, ((string)$parm->name."(".(string)$parm->dbtype.")"));
}
// -----------------
// output row header
// -----------------
...
<script>
<sql>
<prepare><![CDATA[select * from animalq where ID < ? and weight > ?]]></prepare>
<execute>
<parm io='in'>7</parm>
